...
func NewState() *State {
	return &State{
		...
		Phase:         domain.PhaseCharacterCreation,
		TurnCount:     0,
...
// SetPhase 设置游戏阶段
func (s *State) SetPhase(phase domain.GamePhase) {
	s.Phase = phase
}

// GetPhase 获取当前游戏阶段
func (s *State) GetPhase() domain.GamePhase {
	return s.Phase
}
...
func (s *State) StartCombat(participants []*combat.Combatant) {
	...
	s.Phase = domain.PhaseCombat
}

// EndCombat 结束战斗
func (s *State) EndCombat() {
	s.Combat = nil
	s.Phase = domain.PhaseExploration
}
