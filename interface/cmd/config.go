...
import (
	...
	config2 "github.com/zwh8800/cdnd/infrastructure/config"
)
...
如果配置文件不存在，将创建 ~/.cdnd/config.yaml。`,
	Run: func(cmd *cobra.Command, args []string) {
		if err := config2.InitConfigFile(); err != nil {
			fmt.Fprintf(os.Stderr, "Error initializing config: %v\n", err)
			os.Exit(1)
		}
		configPath, _ := config2.GetConfigPath()
...
func printAllConfig(cfg *config2.Config) {
	fmt.Println("当前配置:")
	fmt.Println("======================")
	fmt.Println("\nLLM 设置:")
