...
import (
	...
	ui2 "github.com/zwh8800/cdnd/interface/ui"
)
...
使用 --skip-creation 可跳过角色创建（用于测试）。`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		// 启动角色创建 TUI
		creationModel := ui2.NewCharacterCreationModel()
		...
		// 获取创建的角色
		character := finalModel.(*ui2.CharacterCreationModel).GetCharacter()
		if character == nil {
		...
		// 启动游戏 TUI
		gameModel := ui2.NewGameModel(engine)
