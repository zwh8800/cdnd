...
type CharacterCreationModel struct {
	...
	race      *character2.Race
	subrace   *character2.SubRace
	class     *character2.Class
	subclass  *character2.SubClass
	abilities character2.Attributes
	skills    []character2.SkillType

	availableRaces   []*character2.Race
	availableClasses []*character2.Class
	availableSkills  []character2.SkillType
	abilityPoints    int
...
func NewCharacterCreationModel() *CharacterCreationModel {
	...
	return &CharacterCreationModel{
		step:             StepName,
		nameInput:        ti,
		availableRaces:   character2.GetAllRaces(),
		availableClasses: character2.GetAllClasses(),
		abilityPoints:    72,
		skills:           make([]character2.SkillType, 0),
	}
}
...
func (m *CharacterCreationModel) initAbilityScores() {
	m.abilities = character2.Attributes{
		Strength:     10,
		Dexterity:    10,
		Constitution: 10,
		Intelligence: 10,
	...
	if m.race != nil {
		for ability, bonus := range m.race.AbilityBonuses {
			switch ability {
			case character2.Strength:
				m.abilities.Strength += bonus
			case character2.Dexterity:
				m.abilities.Dexterity += bonus
			case character2.Constitution:
				m.abilities.Constitution += bonus
			case character2.Intelligence:
				m.abilities.Intelligence += bonus
			case character2.Wisdom:
				m.abilities.Wisdom += bonus
			case character2.Charisma:
...
func (m *CharacterCreationModel) initAvailableSkills() {
	m.availableSkills = character2.AllSkillTypes()
}
...
func (m *CharacterCreationModel) renderSkillsStep() string {
	...
	for i, skill := range m.availableSkills {
		name := character2.GetSkillName(skill)
		var line string
...
func (m *CharacterCreationModel) isSkillSelected(skill character2.SkillType) bool {
...
func (m *CharacterCreationModel) GetCharacter() *character2.Character {
	name := m.nameInput.Value()
	if name == "" || m.race == nil || m.class == nil {
		return nil
	}
	c := character2.NewCharacter(name, *m.race, *m.class)
	c.Attributes = m.abilities
	conMod := c.Attributes.Modifier(character2.Constitution)
