...
func (m *GameModel) renderStatusBarCompact(c *character2.Character) string {
	// 左侧：角色信息
	...
	if m.phase == domain.PhaseCombat {
		...
		// 先攻
		initMod := c.Attributes.Modifier(character2.Dexterity)
		if initMod >= 0 {
		...
		// 探索阶段显示位置
		if m.phase == domain.PhaseExploration {
...
func (m *GameModel) getTopSpellSlotsFraction(c *character2.Character) string {
	if c.SpellSlots.IsEmpty() {
		return ""
	}

	// 获取最大法术槽 (根据职业和等级)
	casterType := character2.GetCasterType(c.Class.ID)
	maxSlots := character2.GetSpellSlotsByType(casterType, c.Level)
	...
	for _, level := range levels {
		...
		levelName := character2.GetSpellLevelName(level)
...
func (m *GameModel) renderStatusBarExpanded(c *character2.Character) string {
	// 顶部栏 (紧凑信息)
...
func (m *GameModel) renderStatPanel(c *character2.Character) string {
	abilities := []struct {
		name  string
		value int
		color string
	...
	for _, ab := range abilities {
		mod := c.Attributes.Modifier(character2.Ability(ab.name))
	...
	// AC, 先攻, 速度, 熟练
	statsLine := fmt.Sprintf("AC:%d 先攻:%+d", c.ArmorClass, c.Attributes.Modifier(character2.Dexterity))
...
func (m *GameModel) renderEquipmentPanel(c *character2.Character) string {
	var lines []string
...
func (m *GameModel) renderSpellPanel(c *character2.Character) string {
	var lines []string
	lines = append(lines, GameStyles.PanelTitle.Render("── 法术 ──"))

	casterType := character2.GetCasterType(c.Class.ID)
	if casterType == character2.SpellcastingNone {
...
func (m *GameModel) renderConditionsPanel(c *character2.Character) string {
	var lines []string
...
func (m *GameModel) renderLocationPanel(c *character2.Character) string {
	var lines []string
