...
## 结论
CDND的UI系统以Bubble Tea为核心，实现了清晰的职责分离：UI专注呈现与交互，引擎专注业务与数据。通过统一的样式系统与工具调用循环，既保证了良好的用户体验，也为后续扩展提供了稳定基础。新增的双输入模式、选项解析系统、动态切换功能和**基于lipgloss的自动换行功能**进一步提升了用户交互效率和文本可读性。UI现在直接处理来自LLM响应的选项，移除了对SetOptionsTool工具的依赖，简化了选项管理流程。建议在新增界面组件时遵循现有样式与消息契约，确保一致性与可维护性。
