...
**更新** 新增双输入模式支持，包括文本输入模式和选择模式，以及选项列表系统。
...
UpdateOptions --> Render["updateViewportContent()<br/>viewport.GotoBottom()"]
TickAnim --> Render
Recalc --> Render
TextInput --> Render
SelectEnter --> Render
...
- [internal/ui/game.go:449-534](file://internal/ui/game.go#L449-L534)
...
## 结论
CDND的UI系统以Bubble Tea为核心，实现了清晰的职责分离：UI专注呈现与交互，引擎专注业务与数据。通过统一的样式系统与工具调用循环，既保证了良好的用户体验，也为后续扩展提供了稳定基础。新增的双输入模式、选项列表系统和动态切换功能进一步提升了用户交互效率。建议在新增界面组件时遵循现有样式与消息契约，确保一致性与可维护性。
