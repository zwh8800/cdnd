...
- 辅助方法
  ...
  - updateViewportContent：将lines连接为字符串并设置viewport内容
...
- 内容更新：updateViewportContent将lines以换行连接为字符串并SetContent
...
## 结论
本主界面以Bubble Tea为核心，结合自定义样式与viewport实现，提供了紧凑与展开两种状态栏模式、流畅的剧情输出滚动与自动换行、实时输入与加载动画反馈。通过Engine的工具执行与事件分发，实现了从键盘输入到LLM响应再到工具叙述的完整数据流。建议在历史长度、渲染频率与工具叙述量上进一步优化，以提升性能与用户体验。
