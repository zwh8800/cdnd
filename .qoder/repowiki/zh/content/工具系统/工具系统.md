...
- [internal/tools/combat_tools.go](file://internal/tools/combat_tools.go)
- [internal/combat/combat_state.go](file://internal/combat/combat_state.go)
...
## 更新摘要
**所做更改**
- 新增战斗工具系统，包括开始战斗、管理回合、处理攻击、应用伤害等完整战斗自动化功能
- 新增战斗状态管理和UI集成组件
- 更新系统提示模板，支持战斗阶段的特殊处理
- 扩展工具注册表，包含所有战斗相关工具
- 更新架构图，反映战斗系统的完整集成
...
## 简介
本文件为 CDND 工具系统的完整技术文档，涵盖工具调用机制的设计原理与实现细节，包括工具注册表、参数验证、权限控制与结果处理。文档详细记录了内置工具的功能与实现，包括角色工具、掷骰工具、物品工具、世界工具和**新增的战斗工具系统**的具体用法；阐述了工具系统的扩展机制，允许开发者注册自定义工具；解释了工具调用的安全模型与访问控制；提供了工具开发的最佳实践与 API 规范；记录了工具调用的错误处理与异常恢复机制；包含工具性能监控与调试支持；为工具开发者提供完整的开发指南与示例代码；并解释工具系统与游戏引擎其他组件的集成方式。

## 项目结构
工具系统位于 internal/tools 目录，围绕工具接口、注册表与内置工具实现展开，并通过游戏引擎与状态管理模块进行集成。**新增的战斗工具系统**提供了完整的回合制战斗自动化功能。核心文件如下：
...
- 内置工具实现：
  ...
  - **战斗工具：internal/tools/combat_tools.go**
- 战斗状态管理：internal/combat/combat_state.go
- 怪物管理器：internal/monster/manager.go
- 引擎集成：internal/game/engine.go
- 游戏状态：internal/game/state/state.go
- 角色模型：internal/character/character.go
- 掷骰库：pkg/dice/dice.go
- 规则引擎：internal/rules/engine.go
- 用户界面：internal/ui/game.go
- **战斗面板UI：internal/ui/combat_panel.go**
- **系统提示构建器：internal/llm/prompt/builder.go**
- **提示词模板：internal/llm/prompt/templates.go**
...
ST["游戏状态<br/>state.go"]
SP["系统提示构建器<br/>prompt/builder.go"]
TM["提示词模板<br/>prompt/templates.go"]
END["引擎层"]
end
subgraph "领域模型"
...
- [internal/tools/combat_tools.go:15-27](file://internal/tools/combat_tools.go#L15-L27)
- [internal/combat/combat_state.go:41-52](file://internal/combat/combat_state.go#L41-L52)
- [internal/monster/manager.go:14-24](file://internal/monster/manager.go#L14-L24)
...
- [internal/tools/combat_tools.go:1-667](file://internal/tools/combat_tools.go#L1-L667)
- [internal/combat/combat_state.go:1-53](file://internal/combat/combat_state.go#L1-L53)
- [internal/monster/manager.go:1-233](file://internal/monster/manager.go#L1-L233)
- [internal/game/engine.go:1-988](file://internal/game/engine.go#L1-L988)
...
- 领域模型层：角色、掷骰、规则引擎、**战斗状态**与游戏状态为工具提供数据与业务逻辑支撑。
- 用户界面层：通过引擎解析的选项列表提供上下文相关的操作选项，**新增战斗面板UI**增强战斗体验。
...
### 战斗工具系统
**新增功能**：完整的回合制战斗自动化系统，包括开始战斗、管理回合、处理攻击、应用伤害等功能。
...
**图表来源**
- [internal/tools/combat_tools.go:57-181](file://internal/tools/combat_tools.go#L57-L181)
- [internal/tools/combat_tools.go:230-404](file://internal/tools/combat_tools.go#L230-L404)
- [internal/tools/combat_tools.go:428-471](file://internal/tools/combat_tools.go#L428-L471)
- [internal/tools/combat_tools.go:502-563](file://internal/tools/combat_tools.go#L502-L563)
- [internal/ui/combat_panel.go:12-52](file://internal/ui/combat_panel.go#L12-L52)
...
### 引擎集成与代理循环
- 工具注册
  - 引擎在初始化时注册所有内置工具，包括**新增的战斗工具**，注入状态访问器与规则引擎。
- 代理循环
  - 构建系统提示与历史上下文，调用 LLM 获取工具调用建议。
  - 执行工具调用，收集叙述与事件，循环直至无工具调用或达到最大迭代次数。
  - **新增战斗阶段处理**：在战斗阶段使用专门的系统提示和战斗历史。
- 叙述生成
...
- [internal/game/engine.go:265-291](file://internal/game/engine.go#L265-L291)
...
- **战斗面板集成**
  - **新增**：当引擎处于战斗阶段时，UI 自动渲染战斗面板，显示敌人状态、先攻顺序等信息。
...
UI --> ENG
CP --> ENG
ENG --> SP["系统提示构建器(prompt/builder.go)"]
SP --> TM["提示词模板(prompt/templates.go)"]
```

**图表来源**
- [internal/game/engine.go:22-33](file://internal/game/engine.go#L22-L33)
- [internal/tools/registry.go:10-15](file://internal/tools/registry.go#L10-L15)
- [internal/tools/combat_tools.go:15-19](file://internal/tools/combat_tools.go#L15-L19)
- [internal/combat/combat_state.go:41-52](file://internal/combat/combat_state.go#L41-L52)
- [internal/monster/manager.go:14-24](file://internal/monster/manager.go#L14-L24)
- [internal/ui/combat_panel.go:12-21](file://internal/ui/combat_panel.go#L12-L21)
...
- [internal/game/engine.go:58-76](file://internal/game/engine.go#L58-L76)
- [internal/tools/combat_tools.go:15-27](file://internal/tools/combat_tools.go#L15-L27)
- [internal/game/state/state.go:155-179](file://internal/game/state/state.go#L155-L179)