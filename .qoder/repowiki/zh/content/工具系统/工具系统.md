...
## 更新摘要
**所做更改**
- 新增 SetOptionsTool 工具的详细说明和实现分析
- 更新工具注册表部分，包含新工具的注册流程
- 增强 UI 集成部分，说明选项工具与用户界面的交互机制
- 扩展工具系统架构图，展示选项工具在整个系统中的作用
- 添加选项工具的参数验证和错误处理机制说明
...
## 简介
本文件为 CDND 工具系统的完整技术文档，涵盖工具调用机制的设计原理与实现细节，包括工具注册表、参数验证、权限控制与结果处理。文档详细记录了内置工具的功能与实现，包括角色工具、掷骰工具、物品工具、世界工具与新增的选项工具的具体用法；阐述了工具系统的扩展机制，允许开发者注册自定义工具；解释了工具调用的安全模型与访问控制；提供了工具开发的最佳实践与 API 规范；记录了工具调用的错误处理与异常恢复机制；包含工具性能监控与调试支持；为工具开发者提供完整的开发指南与示例代码；并解释工具系统与游戏引擎其他组件的集成方式。
...
- 内置工具实现：
  ...
  - 选项工具：internal/tools/options_tool.go
...
- 用户界面层：通过选项工具提供上下文相关的操作选项，增强交互体验。
...
### 选项工具
- 设置选项（set_options）
  - 参数：options（字符串数组，每个元素是一个可选操作）。
  - 行为：将玩家当前可用的操作选项存储到游戏状态中，供用户界面显示。
  - 状态访问：通过 StateAccessor.SetCurrentOptions 更新当前选项列表。
  - UI 集成：与用户界面的选项列表组件配合，提供上下文相关的操作选择。

```mermaid
flowchart TD
A["输入参数(options)"] --> B["验证参数类型"]
B --> |有效| C["转换为字符串数组"]
C --> D["调用StateAccessor.SetCurrentOptions"]
D --> E["生成ToolResult"]
B --> |无效| F["返回参数错误"]
E --> G["返回ToolResult"]
F --> H["返回错误"]
...
**图表来源**
- [internal/tools/options_tool.go:38-66](file://internal/tools/options_tool.go#L38-L66)
...
### 引擎集成与代理循环
- 工具注册
  - 引擎在初始化时注册所有内置工具，包括新增的选项工具，注入状态访问器与规则引擎。
- 代理循环
...
## 依赖分析
- 工具到引擎
  - 角色工具、物品工具、世界工具、选项工具依赖 StateAccessor 读写游戏状态。
  - 掷骰工具依赖规则引擎与掷骰库。
- 引擎到工具
...
- UI 集成
  - 用户界面通过选项工具接收当前可用的操作选项，提供上下文相关的交互体验。
...
- [internal/tools/options_tool.go:10](file://internal/tools/options_tool.go#L10)
- [internal/game/state.go:13-42](file://internal/game/state.go#L13-L42)
- [internal/character/character.go:8-61](file://internal/character/character.go#L8-L61)
- [pkg/dice/dice.go:33-41](file://pkg/dice/dice.go#L33-L41)
- [internal/rules/engine.go:8-14](file://internal/rules/engine.go#L8-L14)
...
- 常见错误与处理
  ...
  - 选项工具错误：检查 options 参数是否为字符串数组，确保状态访问器正常工作。
- 调试建议
...
## 结论
CDND 工具系统通过清晰的接口设计、并发安全的注册表与严格的参数验证，实现了可扩展、可维护且安全的工具调用机制。内置工具覆盖角色、掷骰、物品、世界与选项五大类，满足 D&D 场景的核心需求。新增的选项工具显著增强了用户交互体验，通过上下文相关的操作选项提供更自然的游戏流程。引擎层的代理循环与叙述生成增强了交互体验。用户界面与工具系统的深度集成，为玩家提供了直观的操作选择。未来可通过扩展注册表与工具接口，轻松引入自定义工具，同时保持一致的 API 与安全模型。
...
- 选项工具开发
  - 选项工具应专注于提供上下文相关的操作选择。
  - 确保选项数组的字符串格式正确，避免包含特殊字符。
  - 考虑与 UI 组件的兼容性，选项长度适中以便显示。
...
- [internal/tools/registry.go:25-33](file://internal/tools/registry.go#L25-L33)
- [internal/game/engine.go:58-76](file://internal/game/engine.go#L58-L76)
- [internal/tools/options_tool.go:13-19](file://internal/tools/options_tool.go#L13-L19)
- [internal/tools/options_tool.go:21-36](file://internal/tools/options_tool.go#L21-L36)
- [internal/tools/options_tool.go:38-66](file://internal/tools/options_tool.go#L38-L66)