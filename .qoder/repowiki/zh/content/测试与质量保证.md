# 测试与质量保证

<cite>
**本文引用的文件**
- [.golangci.yml](file://.golangci.yml)
- [Makefile](file://Makefile)
- [go.mod](file://go.mod)
- [infrastructure/prompt/builder.go](file://infrastructure/prompt/builder.go)
- [infrastructure/prompt/resp_parser_test.go](file://infrastructure/prompt/resp_parser_test.go)
- [domain/dice/dice_test.go](file://domain/dice/dice_test.go)
- [application/tools/registry.go](file://application/tools/registry.go)
- [application/tools/dice_tools.go](file://application/tools/dice_tools.go)
- [infrastructure/llm/factory.go](file://infrastructure/llm/factory.go)
- [infrastructure/llm/registry.go](file://infrastructure/llm/registry.go)
- [application/engine/engine.go](file://application/engine/engine.go)
- [application/state/state.go](file://application/state/state.go)
- [infrastructure/storage/types.go](file://infrastructure/storage/types.go)
</cite>

## 目录
1. [引言](#引言)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 引言
本文件面向CDND项目的测试与质量保证体系，系统化阐述测试策略与测试金字塔（单元测试、集成测试、端到端测试），并结合仓库中现有的测试实现，给出可操作的质量标准、静态分析与代码规范、持续集成与自动化流程建议、覆盖率要求、性能与基准测试方法、测试数据管理与模拟对象使用指南，以及面向贡献者的测试编写与维护指导。

## 项目结构
- 测试主要分布在以下位置：
  - 提示词解析与颜色标记测试：infrastructure/prompt/resp_parser_test.go
  - 掷骰系统测试：domain/dice/dice_test.go
  - 工具系统测试：application/tools/registry.go、application/tools/dice_tools.go
  - LLM提供者抽象与工厂：infrastructure/llm/factory.go、infrastructure/llm/registry.go
  - 存档与状态：infrastructure/storage/types.go、application/state/state.go
  - 游戏引擎：application/engine/engine.go
- 质量工具与自动化：
  - Makefile 提供测试、覆盖率、静态检查、格式化等常用命令
  - .golangci.yml 定义静态分析规则集与排除规则
  - go.mod 描述依赖版本与模块边界

```mermaid
graph TB
subgraph "测试层"
UT["单元测试<br/>infrastructure/prompt/resp_parser_test.go<br/>domain/dice/dice_test.go"]
IT["集成测试<br/>application/tools/*<br/>infrastructure/llm/*"]
E2E["端到端测试<br/>application/engine/*<br/>infrastructure/storage/*"]
end
subgraph "质量工具"
MK["Makefile 目标<br/>test/test-cover/lint/fmt"]
GC["GolangCI 配置<br/>.golangci.yml"]
MOD["Go 模块<br/>go.mod"]
end
UT --> IT
IT --> E2E
MK --> UT
MK --> IT
MK --> E2E
GC --> MK
MOD --> UT
MOD --> IT
MOD --> E2E
```

**图表来源**
- [Makefile:34-61](file://Makefile#L34-L61)
- [.golangci.yml:1-106](file://.golangci.yml#L1-L106)
- [go.mod:1-55](file://go.mod#L1-L55)

**章节来源**
- [Makefile:1-105](file://Makefile#L1-L105)
- [.golangci.yml:1-106](file://.golangci.yml#L1-L106)
- [go.mod:1-55](file://go.mod#L1-L55)

## 核心组件
- 提示词解析器（Response Parser）
  - 功能：解析颜色标记、提取选项列表、构建系统提示词；支持number/keyword/status/combat/success/danger/quote标记类型
  - 关键测试：颜色标记解析与样式映射、选项解析、文本保留验证
- 掷骰系统（Dice）
  - 功能：提供 d20 标准/优势/劣势掷骰、表达式解析、成功判定、暴击识别
  - 关键测试：验证范围、表达式解析、优势/劣势逻辑、暴击判定
- 工具系统（Tools）
  - 功能：注册表管理工具、工具参数定义、工具执行（投骰、技能检定、豁免检定、造成/治疗伤害、状态增删等）
  - 关键测试：工具注册与执行、参数校验、叙事输出与数据返回
- LLM提供者抽象与工厂
  - 功能：统一的消息结构、请求/响应、流式分片；按配置创建不同提供者（OpenAI、Anthropic、Ollama）
  - 关键测试：提供者创建、参数映射、工具定义导出
- 存档与状态
  - 功能：存档数据结构、状态读写、回合数与标志/计数器管理、战斗初始化
  - 关键测试：状态导入/导出、标志/计数器一致性
- 游戏引擎
  - 功能：工具注册、状态管理、游戏流程控制
  - 关键测试：引擎初始化、工具执行、状态同步

**章节来源**
- [infrastructure/prompt/builder.go:1-331](file://infrastructure/prompt/builder.go#L1-L331)
- [infrastructure/prompt/resp_parser_test.go:1-330](file://infrastructure/prompt/resp_parser_test.go#L1-L330)
- [domain/dice/dice_test.go:1-204](file://domain/dice/dice_test.go#L1-L204)
- [application/tools/registry.go:1-109](file://application/tools/registry.go#L1-L109)
- [application/tools/dice_tools.go:1-314](file://application/tools/dice_tools.go#L1-L314)
- [infrastructure/llm/factory.go:1-70](file://infrastructure/llm/factory.go#L1-L70)
- [infrastructure/llm/registry.go:1-140](file://infrastructure/llm/registry.go#L1-L140)
- [application/engine/engine.go:54-150](file://application/engine/engine.go#L54-L150)
- [application/state/state.go:105-164](file://application/state/state.go#L105-L164)
- [infrastructure/storage/types.go:110-147](file://infrastructure/storage/types.go#L110-L147)

## 架构总览
下图展示测试金字塔在CDND中的落地：单元测试覆盖核心算法与工具；集成测试覆盖工具注册与执行、LLM提供者创建与消息交互；端到端测试覆盖存档/状态与游戏引擎主流程。

```mermaid
graph TB
subgraph "单元测试层"
U1["提示词解析器测试<br/>infrastructure/prompt/resp_parser_test.go"]
U2["掷骰系统测试<br/>domain/dice/dice_test.go"]
U3["工具参数与执行测试<br/>application/tools/*"]
end
subgraph "集成测试层"
I1["工具注册与执行<br/>application/tools/registry.go<br/>application/tools/dice_tools.go"]
I2["LLM提供者工厂/注册表<br/>infrastructure/llm/factory.go<br/>infrastructure/llm/registry.go"]
I3["游戏引擎工具注册<br/>application/engine/engine.go"]
end
subgraph "端到端测试层"
E1["存档/状态读写<br/>infrastructure/storage/types.go<br/>application/state/state.go"]
E2["完整游戏流程建议<br/>engine + tools + LLM"]
end
U1 --> I1
U2 --> I1
U3 --> I1
I1 --> E1
I2 --> E1
I3 --> E1
E1 --> E2
```

**图表来源**
- [infrastructure/prompt/resp_parser_test.go:1-330](file://infrastructure/prompt/resp_parser_test.go#L1-L330)
- [domain/dice/dice_test.go:1-204](file://domain/dice/dice_test.go#L1-L204)
- [application/tools/registry.go:1-109](file://application/tools/registry.go#L1-L109)
- [application/tools/dice_tools.go:1-314](file://application/tools/dice_tools.go#L1-L314)
- [infrastructure/llm/factory.go:1-70](file://infrastructure/llm/factory.go#L1-L70)
- [infrastructure/llm/registry.go:1-140](file://infrastructure/llm/registry.go#L1-L140)
- [application/engine/engine.go:54-150](file://application/engine/engine.go#L54-L150)
- [infrastructure/storage/types.go:110-147](file://infrastructure/storage/types.go#L110-L147)
- [application/state/state.go:105-164](file://application/state/state.go#L105-L164)

## 详细组件分析

### 提示词解析器测试
**更新** 提示词构建器测试已迁移至resp_parser_test.go，包含颜色标记解析、选项解析和样式映射验证。

- 测试要点
  - 颜色标记解析：number/keyword/status/combat/success/danger/quote 类型解析与样式应用
  - 样式映射完整性：确保每种标记类型在ColorMarkerStyles中有对应映射
  - 选项解析：从文本中提取选项列表，支持多种格式和边界情况
  - 文本保留：解析后应保留非标记文本
- 测试策略
  - 参数化用例覆盖多种标记组合与边界情况（空内容、嵌套括号、未知类型）
  - 选项解析测试覆盖标准格式、特殊字符、空选项、大量选项等场景
  - 断言样式应用后的输出不包含原始标记（除特殊用例外）

```mermaid
sequenceDiagram
participant T as "测试用例"
participant P as "ParseColorMarkers"
participant S as "样式映射"
T->>P : 输入包含标记的文本
P->>S : 查找标记类型对应的样式
S-->>P : 返回样式或回退内容
P-->>T : 输出带样式的文本
T->>T : 断言包含预期内容且不残留原始标记
```

**图表来源**
- [infrastructure/prompt/builder.go:28-49](file://infrastructure/prompt/builder.go#L28-L49)
- [infrastructure/prompt/resp_parser_test.go:9-124](file://infrastructure/prompt/resp_parser_test.go#L9-L124)

**章节来源**
- [infrastructure/prompt/builder.go:14-49](file://infrastructure/prompt/builder.go#L14-L49)
- [infrastructure/prompt/resp_parser_test.go:9-124](file://infrastructure/prompt/resp_parser_test.go#L9-L124)

### 掷骰系统测试
- 测试要点
  - 基础掷骰：n个s面骰的范围校验、数量校验
  - d20扩展：带调整值、优势/劣势、暴击/大失败识别
  - 表达式解析：支持 d20、1d20+5、1d20adv/dis 等
  - 成功判定：根据DC判断成功/失败
- 测试策略
  - 边界与异常：空表达式、非法字符串、空参数
  - 随机性验证：多次运行统计自然20/1出现比例，确保逻辑正确

```mermaid
flowchart TD
Start(["开始"]) --> Parse["解析表达式"]
Parse --> ParseOK{"解析成功？"}
ParseOK --> |否| ReturnErr["返回错误"]
ParseOK --> |是| RollType{"是否d20优势/劣势？"}
RollType --> |是| D20Adv["D20WithModifier<br/>选择高/低值"]
RollType --> |否| StdRoll["Roll n,d<br/>sum+modifier"]
D20Adv --> CritCheck["检查自然20/1"]
StdRoll --> CritCheck
CritCheck --> Success{"Total>=DC？"}
Success --> |是| MarkSuccess["标记成功"]
Success --> |否| MarkFail["标记失败"]
MarkSuccess --> End(["结束"])
MarkFail --> End
ReturnErr --> End
```

**图表来源**
- [domain/dice/dice.go:43-158](file://domain/dice/dice.go#L43-L158)
- [domain/dice/dice_test.go:7-204](file://domain/dice/dice_test.go#L7-L204)

**章节来源**
- [domain/dice/dice.go:9-158](file://domain/dice/dice.go#L9-L158)
- [domain/dice/dice_test.go:7-204](file://domain/dice/dice_test.go#L7-L204)

### 工具系统测试
- 测试要点
  - 工具注册表：并发安全、工具存在性、权限阶段控制
  - 投骰工具：参数校验、表达式解析、叙事输出与数据返回
  - 技能检定工具：参数校验、角色状态验证、规则引擎集成
  - 豁免检定工具：参数校验、角色状态验证、规则引擎集成
- 测试策略
  - 使用最小状态桩（StateAccessor）模拟角色状态，避免外部依赖
  - 对工具执行路径进行参数化测试，覆盖必填项缺失、数值越界等

```mermaid
classDiagram
class Registry {
+Register(tool, allowedPhases)
+Get(name) Tool,bool
+Execute(ctx,name,args) ToolResult,error
+GetToolDefinitions() []*ToolDefinition
+ListTools() []string
+HasTool(name) bool
+IsAllowedInPhase(name,phase) bool
}
class RollDiceTool {
+Parameters() map
+Execute(ctx,args) *ToolResult,error
}
class SkillCheckTool {
+Parameters() map
+Execute(ctx,args) *ToolResult,error
}
class SavingThrowTool {
+Parameters() map
+Execute(ctx,args) *ToolResult,error
}
Registry --> RollDiceTool : "管理"
Registry --> SkillCheckTool : "管理"
Registry --> SavingThrowTool : "管理"
```

**图表来源**
- [application/tools/registry.go:10-109](file://application/tools/registry.go#L10-L109)
- [application/tools/dice_tools.go:12-71](file://application/tools/dice_tools.go#L12-L71)
- [application/tools/dice_tools.go:74-198](file://application/tools/dice_tools.go#L74-L198)
- [application/tools/dice_tools.go:200-313](file://application/tools/dice_tools.go#L200-L313)

**章节来源**
- [application/tools/registry.go:25-116](file://application/tools/registry.go#L25-L116)
- [application/tools/dice_tools.go:38-71](file://application/tools/dice_tools.go#L38-L71)
- [application/tools/dice_tools.go:137-198](file://application/tools/dice_tools.go#L137-L198)
- [application/tools/dice_tools.go:252-313](file://application/tools/dice_tools.go#L252-L313)

### LLM提供者抽象与工厂测试
- 测试要点
  - Provider接口：Generate/GenerateStream/SetModel/SetMaxTokens/SetTemperature
  - 工厂：根据配置创建OpenAI/Anthropic/Ollama提供者，参数映射正确
  - 注册表：提供者注册、默认提供者切换、列表查询
- 测试策略
  - 使用最小配置桩，验证工厂返回的Provider满足接口契约
  - 验证未知提供者名称时的错误处理

```mermaid
sequenceDiagram
participant C as "配置"
participant F as "NewProvider"
participant P as "Provider实例"
C->>F : 提供默认提供者名称与配置
F->>F : 参数映射到ProviderConfig
F->>P : 根据名称创建具体Provider
P-->>F : 返回Provider实例
F-->>C : 返回Provider
```

**图表来源**
- [infrastructure/llm/factory.go:9-70](file://infrastructure/llm/factory.go#L9-L70)
- [infrastructure/llm/registry.go:22-139](file://infrastructure/llm/registry.go#L22-L139)

**章节来源**
- [infrastructure/llm/factory.go:9-70](file://infrastructure/llm/factory.go#L9-L70)
- [infrastructure/llm/registry.go:22-139](file://infrastructure/llm/registry.go#L22-L139)

### 存档与状态测试
- 测试要点
  - SaveData结构：角色、世界、场景、NPC、历史、战斗状态等字段
  - State：回合数、标志/计数器、任务、战斗初始化
  - 引擎：工具注册、状态导入/导出
- 测试策略
  - 构造最小SaveData样本，验证导入后角色与世界数据一致
  - 验证标志/计数器的增减与查询

```mermaid
flowchart TD
A["构造SaveData样本"] --> B["引擎导入"]
B --> C{"角色加载成功？"}
C --> |是| D["世界数据导入"]
C --> |否| E["返回错误"]
D --> F["状态同步完成"]
F --> G["后续流程测试"]
```

**图表来源**
- [infrastructure/storage/types.go:110-147](file://infrastructure/storage/types.go#L110-L147)
- [application/engine/engine.go:124-150](file://application/engine/engine.go#L124-L150)
- [application/state/state.go:105-164](file://application/state/state.go#L105-L164)

**章节来源**
- [infrastructure/storage/types.go:110-147](file://infrastructure/storage/types.go#L110-L147)
- [application/engine/engine.go:124-150](file://application/engine/engine.go#L124-L150)
- [application/state/state.go:105-164](file://application/state/state.go#L105-L164)

## 依赖分析
- 模块依赖与测试耦合
  - 提示词解析器依赖样式库与角色/世界模型，测试应隔离样式库，重点验证解析与上下文拼装
  - 掷骰系统依赖加密安全随机源，测试关注范围与逻辑分支覆盖
  - 工具系统依赖规则引擎与状态访问器，测试通过桩对象模拟
  - LLM提供者工厂依赖配置模块，测试通过最小配置桩
- 质量工具
  - Makefile提供统一入口，.golangci.yml定义规则，go.mod约束依赖版本

```mermaid
graph LR
GC[".golangci.yml"] --> MK["Makefile"]
MOD["go.mod"] --> UT["单元测试"]
MOD --> IT["集成测试"]
MOD --> E2E["端到端测试"]
MK --> UT
MK --> IT
MK --> E2E
```

**图表来源**
- [.golangci.yml:1-106](file://.golangci.yml#L1-L106)
- [Makefile:34-61](file://Makefile#L34-L61)
- [go.mod:1-55](file://go.mod#L1-L55)

**章节来源**
- [.golangci.yml:1-106](file://.golangci.yml#L1-L106)
- [Makefile:34-61](file://Makefile#L34-L61)
- [go.mod:1-55](file://go.mod#L1-L55)

## 性能考虑
- 单元测试
  - 使用参数化测试减少重复代码，提高覆盖度
  - 对随机性逻辑进行多次采样统计，而非依赖单一结果
- 集成测试
  - 使用内存态的最小状态桩，避免IO与外部服务
  - 对工具执行路径进行轻量回归，聚焦关键分支
- 端到端测试
  - 优先覆盖关键流程（存档/读档、工具链、LLM交互），逐步扩展
- 性能基准
  - 建议为提示词解析器与掷骰系统增加基准测试，监控热点路径性能变化

## 故障排查指南
- 测试失败定位
  - 单元测试：查看参数化用例与断言，确认输入/期望输出
  - 集成测试：检查工具注册顺序、参数解析、状态访问器是否为空
  - LLM测试：核对配置、API密钥、模型名称与最大token设置
- 覆盖率问题
  - 使用覆盖率报告定位未覆盖分支，补充边界用例
- 静态检查
  - 遵循GolangCI规则，修复告警后再提交

**章节来源**
- [Makefile:34-41](file://Makefile#L34-L41)
- [.golangci.yml:81-105](file://.golangci.yml#L81-L105)

## 结论
CDND当前已具备完善的测试基础：提示词解析器与掷骰系统的单元测试覆盖核心逻辑；工具系统与LLM提供者抽象形成清晰的集成测试边界；存档/状态与游戏引擎为主流程提供端到端测试入口。建议进一步完善端到端测试矩阵与性能基准，持续提升覆盖率与稳定性。

## 附录

### 测试金字塔与实施建议
- 单元测试（建议覆盖率≥80%）
  - 提示词解析器：颜色标记解析、样式映射、选项解析、上下文拼装
  - 掷骰系统：表达式解析、优势/劣势、暴击判定、成功判定
  - 工具系统：参数校验、执行路径、叙事输出
- 集成测试（建议覆盖率≥60%）
  - 工具注册表并发安全、权限控制
  - LLM提供者工厂与注册表
  - 存档/状态导入/导出与引擎主流程
- 端到端测试（建议覆盖率≥40%）
  - 完整游戏会话（角色创建→场景探索→战斗→存档/读档）
  - 工具链与LLM交互闭环

### 代码质量标准与GolangCI配置
- 已启用的静态检查器：errcheck、gosimple、govet、ineffassign、staticcheck、unused、gofmt、goimports、misspell、revive、prealloc、typecheck、gocritic、dupl、exportloopref
- 测试文件排除规则：测试文件不运行gocyclo、errcheck、dupl、gosec
- 本地前缀与风格：goimports本地包前缀为github.com/zwh8800/cdnd

**章节来源**
- [.golangci.yml:5-106](file://.golangci.yml#L5-L106)

### 持续集成与自动化测试流程
- 常用命令
  - 测试与覆盖率：make test；生成HTML报告：make test-cover
  - 静态检查：make lint
  - 代码格式化：make fmt
- 建议在CI中执行
  - go mod tidy
  - make lint
  - make test -race -coverprofile=coverage.out ./...
  - 上传覆盖率报告至平台（如codecov）

**章节来源**
- [Makefile:34-61](file://Makefile#L34-L61)

### 测试最佳实践
- 用例设计
  - 参数化与边界用例优先
  - 分离"行为"与"状态"的断言
- 模拟对象与桩
  - 使用最小状态桩（StateAccessor）与配置桩
  - 对外部依赖（LLM、随机源）进行可控替换
- 数据管理
  - 将测试数据放入 testdata 目录，便于维护与复用
- 并发与竞态
  - 使用 -race 标志运行测试，发现潜在竞态

### 覆盖率要求
- 单元测试：≥80%
- 集成测试：≥60%
- 端到端测试：≥40%
- 建议在CI中设置覆盖率阈值并阻断低于阈值的合并

### 性能测试与基准测试
- 基准测试
  - 为提示词解析器与掷骰系统添加Benchmark，定期回归
- 压力测试
  - 在集成测试中模拟高并发工具调用与LLM请求

### 测试环境搭建与工具使用
- 环境准备
  - 安装golangci-lint：make lint 会自动安装
  - 使用go.mod管理依赖：make deps / make tidy
- 常用命令
  - make test：运行全部测试并生成覆盖率
  - make test-cover：生成HTML覆盖率报告
  - make lint：运行静态检查
  - make fmt：格式化代码

**章节来源**
- [Makefile:53-61](file://Makefile#L53-L61)
- [go.mod:1-55](file://go.mod#L1-L55)